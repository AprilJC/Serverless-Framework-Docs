通过 [Serverless Framework Koa 组件](https://github.com/serverless-components/tencent-koa)，可以快速实现 Koa 传统应用从本地到 Serverless 函数平台的迁移。
...
Koa 组件将在腾讯云账号中使用到如下 Serverless 服务：

- **API 网关**：API 网关将会接收外部请求并且转发到 SCF 云函数中。
- **SCF 云函数**：云函数将承载 Koa 应用。
